        ...
        <form class="mt-8 space-y-6" [formGroup]="registerForm" (ngSubmit)="onSubmit()">
          <div class="rounded-md shadow-sm -space-y-px">
            <div>
              ...
              <input id="name" name="name" type="text" formControlName="name" required class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-t-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" placeholder="Full Name">
            </div>
            <div>
            ...
            <div>
              ...
              <div *ngIf="registerForm.errors?.['passwordMismatch'] && registerForm.get('confirmPassword')?.touched" class="text-red-500 text-xs mt-1 ml-1">
                Passwords do not match
              </div>
            </div>
            <div>
              <label for="employeeId" class="sr-only">Old Employee ID</label>
              <input id="employeeId" name="employeeId" type="text" formControlName="employeeId" required class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-b-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" placeholder="Old Employee ID">
            </div>
          </div>
          ...
          <div>
            <button 
              ...
              [ngClass]="{'opacity-50 cursor-not-allowed': registerForm.invalid || isLoading}"
            >
...
export class RegisterComponent {
  ...
  async onSubmit() {
    ...
    try {
      const { name, email, password, employeeId } = this.registerForm.value;
      ...
      if (error.code === 'auth/email-already-in-use') {
        ...
        this.errorMessage = 'The password is too weak. Please use a stronger password.';
      } else {
  ...
  async registerWithGoogle() {
    ...
    try {
      // For social logins, we'll first authenticate the user
      const result = await this.userService.signInWithGoogle();
      
      // Then redirect to a profile completion page where they can add their employee ID
      ...
      if (error.code === 'auth/popup-closed-by-user') {
        this.errorMessage = 'Registration was cancelled. Please try again.';
      } else {
        this.errorMessage = 'An error occurred during registration with Google. Please try again later.';
      }
    } finally {
      this.isLoading = false;
    }
  ...
  async registerWithGithub() {
    ...
    try {
      ...
      if (error.code === 'auth/popup-closed-by-user') {
        ...
        this.errorMessage = 'An error occurred during registration with GitHub. Please try again later.';
      }
    } finally {
      this.isLoading = false;
    }
